...
import (
	...
	pb "github.com/kzmake/_akka-grpc-with-dapr/gateway/zz_generated/go/agwd/user/v1"
)

type Env struct {
	GateawayAddress string `default:"0.0.0.0:8080"`
	ServiceAddress  string `default:"localhost:50001"`
}
...
func newGatewayServer(ctx context.Context) (*http.Server, error) {
	...
	mux := runtime.NewServeMux(
		...
		runtime.WithMetadata(func(_ context.Context, _ *http.Request) metadata.MD {
			return metadata.Pairs("dapr-app-id", "user")
		}),
	)

	if err := pb.RegisterUserServiceHandlerFromEndpoint(ctx, mux, env.ServiceAddress, []grpc.DialOption{grpc.WithInsecure()}); err != nil {
